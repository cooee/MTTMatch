 ...
 */
function calculateLeaf(address: string, rank: number): string {
 ...
 */
function buildMerkleTree(winners: Winner[]): {
    root: string;
    proofs: Map<string, string[]>;
    leaves: Map<string, string>;
} {
    // 计算所有叶子节点
    const leafNodes: { address: string; rank: number; hash: string }[] = winners.map(w => ({
        address: w.address,
        rank: w.rank,
 ...
 */
function formatProofForRemix(proof: string[]): string {
    if (proof.length === 0) {
        return '[]';
    }
    return '["' + proof.join('","') + '"]';
 ...
 * 
 * 1. 在终端运行:
 *    npx ts-node scripts/calculate_merkle.ts
 * 
 * 2. 或者在 Remix 控制台复制粘贴此代码（去掉 import 语句，使用全局的 ethers）
 * 
 * 3. 修改配置：
